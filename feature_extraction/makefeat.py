...
class featureextract:
    ...
    def extract_features(self, filepath):
        ...
        if self.mode == 'mfcc':
            audio_features = self.compute_all_mfcc(audio_file, self.sample_rate, self.n_fft, self.hop_length, self.n_mfcc, self.deltas)
        elif self.mode == 'stft':
            audio_features = self.compute_all_stft(audio_file, self.sample_rate, self.n_fft, self.hop_length)
        elif self.mode == 'mel-spectogram':
            audio_features = self.compute_all_mel_spectogram(audio_file, self.sample_rate, self.n_fft, self.hop_length)
    ...
    @staticmethod
    def compute_all_mel_spectogram(audio_file, sample_rate, n_fft, hop_length):
    ...
    @staticmethod
    def compute_all_stft(audio_file, sample_rate, n_fft, hop_length):
        return librosa.stft(audio_file, n_fft=n_fft, hop_length=hop_length)


    @staticmethod
    def compute_all_mfcc(audio_file, sample_rate, n_fft, hop_length, n_mfcc, deltas=False):
